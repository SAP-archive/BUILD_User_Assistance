            ...
            for (j = 0; j < iLen; j++) {
                if (typeof (aData[j]) != 'object') {
                    ...
                    for (k = 0; k < strings.length; k++){
                        string = strings[k];
                        string = string.replace(/^(<.[^(><.)]+>|\s)*/g, ''); //strip leading tags and spaces
						string = string.replace(/(<.[^(><.)]+>|\s)*$/g, ''); //strip trailing tags and spaces
						...
						escaped = escaped.replace(/&&&plus/g, "\\+");
						escaped = "(^|;)(<.[^(><.)]+>|\\s)*?" + escaped + "(<.[^(><.)]+>|\\s)*?(;|$)";
                        optionObject = optionObject.replace(/&&&lt/g, "&lt;");
                        optionObject = optionObject.replace(/&&&gt/g, "&gt;");
						optionObject = optionObject.replace(/&&&plus/g, "+");
                            var selected = '';
                            if (escaped == currentFilter
                                || escaped == escape(currentFilter)
                                )
                                selected = 'selected '
                            stringToAdd = '<option ' + selected + ' value="' + escaped + '">' + optionObject + '</option>';
							if (!blank && optionObject === localization.sEmptyCells) {
								r += stringToAdd;
								blank = true;
							} else if (blank && optionObject === localization.sEmptyCells) {
								
							}
                            else if (r.indexOf(stringToAdd) === -1) {
                                r += stringToAdd;
                            }
                        }
                     }
